...
class DatasetManager:
    ...
    def merge_datasets(
        self, dataset_paths_str: str, output_dir: Path, chunk_name: str, verbose: bool = True
    ):
        """
    ...
    def _merge_all_meta_files(
        ...
        for i, dataset_path in enumerate(dataset_paths):
            ...
            if src_info.exists():
                ...
                info_out.write_text(json.dumps(merged_info, ensure_ascii=False, indent=2))
            ...
            try:
                ...
                if isinstance(meta_info.get("splits"), dict) and isinstance(
                    ...
                    try:
                        ...
                        pass
                info_path.write_text(json.dumps(meta_info, ensure_ascii=False, indent=2))
            except Exception as e:
        ...
        for item in items_to_process:
            ...
            if is_json_list_format:
                ...
                if is_decodable_json:  # If it was JSON, serialize it back
                    new_data_list_for_output.append(json.dumps(obj_to_process, ensure_ascii=False, separators=(",", ":")))
                else:  # If it was not JSON (e.g. malformed line), keep original line
                    new_data_list_for_output.append(raw_line_if_not_json)

        # Write back to file
        if is_json_list_format:
            path.write_text(json.dumps(new_data_list_for_output, ensure_ascii=False, indent=2) + "\n")
        else:  # JSONL
