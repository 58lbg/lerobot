...
def move_to_zero_position(robot, duration=3.0, kp=0.5):
    ...
    control_freq = 30  # 50Hz control frequency
    ...
    for step in range(total_steps):
        ...
        if step_print % 15 == 0:
            step_print = 0
            print(f"move zero current : {current_positions}")
            print(f"move zero change pos: {robot_action}")
...
def p_control_loop(robot, keyboard, target_positions, start_positions, current_x, current_y, kp=0.5, control_freq=30):
    """
    P control loop
    ...
    while True:
        try:
            ...
            #     print(f"change pos: {robot_action}")
...
def main():
    ...
    try:
        ...
        from lerobot.common.robots.lekiwi_car import LeKiwiCarClient, LeKiwiCarClientConfig
        from lerobot.common.teleoperators.keyboard import KeyboardTeleop, KeyboardTeleopConfig

        # Configure robot
        robot_config = LeKiwiCarClientConfig(remote_ip="10.253.58.135", id="lbg_kiwi_car")
        robot = LeKiwiCarClient(robot_config)
